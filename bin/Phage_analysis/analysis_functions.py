...
def get_taxon_sample_dataframe(sample_taxon2hitcounts_normalized_count, taxon2_avg_count, 
    ...
    taxon_id_sample_normalized_count = sample_taxon2hitcounts_normalized_count.merge(taxon2_avg_count_filtered, how = 'inner', left_on = 'taxon_id', right_on = 'taxon_id')[['sample_id', 'taxon_name_x', 'normalized_count']].drop_duplicates()
    ...
    random_numbers = pandas.DataFrame(mic_factor*np.random.rand(taxon_id_sample_normalized_count_pivot.shape[0], taxon_id_sample_normalized_count_pivot.shape[1]), columns =taxon_id_sample_normalized_count_pivot.columns, index=taxon_id_sample_normalized_count_pivot.index )
...
def plot_cluster_map_with_classlabel(classlabel, data_table, data_labels, all_colors, pdffile=None, fig_size_y_per_entitity=0.3):
    ...
    legend_data.plot.barh(color=used_colors, ax=f.gca())
